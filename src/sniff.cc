...
int (*my_printf)(const char *__restrict path, ...);
#undef open
int open(const char *pathname, int flags, mode_t mode) {
  if (my_open == NULL) my_open = reinterpret_cast<decltype(my_open)>(dlsym(RTLD_NEXT, "open"));
  if (my_printf == NULL) my_printf = reinterpret_cast<decltype(my_printf)>(dlsym(RTLD_NEXT, "printf"));
  int ret = my_open(pathname, flags, mode);
  my_printf("open %s (0o%o) = %d\n", pathname, flags, ret);
  files[ret] = pathname;
  return ret;
}
